...
public class SandStarOreGenerator implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
        if (world.provider instanceof WorldProviderSurface) {
            ...
            if (BiomeDictionary.hasType(world.getBiome(pos), BiomeDictionary.Type.MOUNTAIN)) {
    ...
    private void generateOre(World world, Random random, int x, int z) {
        ...
        if (world.provider instanceof WorldProviderSurface) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                int genY = 10 + random.nextInt(120);
                int genZ = z + random.nextInt(16);
                new WorldGenMinable(
                        JapariBlocks.SANDSTAR_ORE.getDefaultState(), 3 + random.nextInt(6)).generate(world, random, new BlockPos(genX, genY, genZ));
            }
        }
    }
}